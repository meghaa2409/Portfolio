    ...
    try {
      ...
      const update = (t) => {
        ...
        if (enableVoiceControl && micReady) {
          ...
          currentRot += dt * (voiceLevel > 0.05 ? voiceRotationSpeed : idleRotationSpeed)
          ...
          // No microphone hooked up — keep a gentle idle rotation so the
          // orb still reads as alive rather than a frozen frame.
      ...
      console.error('Error initializing VoiceOrb:', err)
      if (container?.firstChild) container.removeChild(container.firstChild)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [hue, enableVoiceControl, voiceSensitivity, maxRotationSpeed, maxHoverIntensity])

  return <div ref={ctnDom} className={`voice-orb ${className}`.trim()} />
}
